...
import { useGetApiV1OwnerGroupBuysManage } from '@moongchijang/api-client/hooks/owner/owner';
import { usePostApiV1PickupsQrCodeVerify } from '@moongchijang/api-client/hooks/pickup/pickup';
import { GetApiV1OwnerGroupBuysManageFilter as Filter } from '@moongchijang/api-client/generated/api.schemas';
import {
  QrScannerModal,
  type ScanToastItem,
...
export function ManagementClient() {
  ...
  const addToast = useCallback(
    (type: ScanToastItem['type'], title: string, subtitle: string) => {
      ...
      setToasts((prev) => [
        ...prev,
        { id, type, title, subtitle, visible: true },
      ]);
      setTimeout(() => {
        setToasts((prev) =>
