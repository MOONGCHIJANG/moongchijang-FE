...
export const getGetApiV1AdminGroupBuyRequestsRequestIdResponseMock = (overrideResponse: Partial<Extract<ApiResponseAdminRequestDetail, object>> = {}): ApiResponseAdminRequestDetail => ({success: faker.datatype.boolean(), data: {requestId: faker.number.int(), storeName: faker.string.alpha({length: {min: 10, max: 20}}), productName: faker.string.alpha({length: {min: 10, max: 20}}), desiredQuantity: faker.number.int(), desiredPickupDate: faker.date.past().toISOString().slice(0, 10), additionalNote: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.string.alpha({length: {min: 10, max: 20}}), null]), null]), status: faker.helpers.arrayElement(['IN_REVIEW','IN_CONTACT','OPENED','REJECTED'] as const), rejectionReason: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.string.alpha({length: {min: 10, max: 20}}), null]), null]), requesterName: faker.string.alpha({length: {min: 10, max: 20}}), createdAt: faker.date.past().toISOString().slice(0, 19) + 'Z'}, error: {}, ...overrideResponse})
