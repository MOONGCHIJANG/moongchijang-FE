...
export const getGetApiV1OwnerGroupBuysManageResponseMock = (overrideResponse: Partial<Extract<ApiResponseOwnerGroupBuyManageList, object>> = {}): ApiResponseOwnerGroupBuyManageList => ({success: faker.datatype.boolean(), data: Array.from({ length: faker.number.int({min: 1, max: 10}) }, (_, i) => i + 1).map(() => ({groupBuyId: faker.number.int(), productName: faker.string.alpha({length: {min: 10, max: 20}}), price: faker.number.int(), pickupDate: faker.date.past().toISOString().slice(0, 10), deadlineDday: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.number.int(), null]), undefined]), achievementRate: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.number.int(), null]), undefined]), currentQuantity: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.number.int(), null]), undefined]), targetQuantity: faker.helpers.arrayElement([faker.helpers.arrayElement([faker.number.int(), null]), undefined]), status: faker.helpers.arrayElement(['ALL','IN_PROGRESS','ACHIEVED','ENDED','PENDING_APPROVAL'] as const)})), error: {}, ...overrideResponse})

export const getGetApiV1OwnerGroupBuysGroupBuyIdManageInProgressResponseMock = (overrideResponse: Partial<Extract<ApiResponseOwnerGroupBuyManageDetail, object>> = {}): ApiResponseOwnerGroupBuyManageDetail => ({success: faker.datatype.boolean(), data: {groupBuyId: faker.number.int(), status: faker.helpers.arrayElement(['ALL','IN_PROGRESS','ACHIEVED','ENDED','PENDING_APPROVAL'] as const), participantSummary: {totalCount: faker.number.int(), completedCount: faker.number.int(), waitingCount: faker.number.int()}, participants: Array.from({ length: faker.number.int({min: 1, max: 10}) }, (_, i) => i + 1).map(() => ({name: faker.string.alpha({length: {min: 10, max: 20}}), phoneNumber: faker.string.alpha({length: {min: 10, max: 20}}), productName: faker.string.alpha({length: {min: 10, max: 20}}), quantity: faker.number.int(), paymentMethod: faker.string.alpha({length: {min: 10, max: 20}}), paymentStatus: faker.string.alpha({length: {min: 10, max: 20}}), pickupTime: faker.string.alpha({length: {min: 10, max: 20}})}))}, error: {}, ...overrideResponse})
